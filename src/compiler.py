...
def add_gen_in_def(code_lines):
    ...
    if is_inside_gen:
        addError("Compiler", "generator method at the end of file {}".format(file))

    return new_lines


def translate_file_contents(file, code, indent_str="    "):
...
""".format(file, indent_str).splitlines()
    ...
    code_lines = add_gen_in_def(code_lines)
...
def generate_desugared_src(hlir, hlir16):
    ...
    for fromfile in glob.glob("src/utils/*.sugar.py"):
        ...
        with open(fromfile, "r") as orig_file:
            code = orig_file.read()
            code = translate_file_contents(fromfile, code)
