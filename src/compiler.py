...
def translate_line_with_insert(file, line_idx, line, indent_str):
    ...
    extra_content = f" // ({prepend_append_funname}ed) {print_file(file)}:{line_idx}\\n" if maybe_pre else ""
...
def print_file(file):
    ...
    noext = nopath.replace(r'[.].*', '')

    if "noext" in args['filestyle']: return noext
    if "nopath" in args['filestyle']: return nopath
    return file
...
""".format(print_file(file), compiler_common.file_indent_str, os.path.relpath(file, relpath) if relpath is not None else file).splitlines()
...
def init_args():
    ...
    parser.add_argument('-filestyle', help='Filename style in generated code comments', required=False, default=[], action='append')
