...
def sugar(file, line):
    ...
    if file_sugar_style[-1] == 'line_comment':
        if file == "{2}":
            return ' // @' + str(line+1) + '\\n'
        return ' // ' + sugar_file + '@' + str(line+1) + '\\n'
    if file_sugar_style[-1] == 'inline_comment':
        if file == "{2}":
            return '\\n/*@' + str(line+1) + '*/'
        return '\\n/*' + sugar_file + '@' + str(line+1) + '*/'
    return ''
...
""".format(file, indent_str, os.path.basename(genfile)).splitlines()
    ...
    for idx, code_line in code_lines:
        ...
        if new_line.strip() != "" and new_line.strip()[-1] != '\\' and idx is not None:
            # TODO idx is sometimes off by one?
            new_line += " ## {} {}".format(file, int(idx) + 1)
...
def print_with_backtrace(exc_type, exc, tb):
    if not pkgutil.find_loader('backtrace'):
        raise

    import backtrace

    relpath_tb = [(os.path.relpath(t1), t2, t3, t4) for (t1, t2, t3, t4) in tb]
...
def detailed_error_message(genfile, file, exc_type, exc, tb):
    ...
    try:
        with open(genfile) as genlines:
            from itertools import islice

            [genline] = islice(genlines, genlineno-1, genlineno)
            file   = genline.split(" ")[-2]
            lineno = int(genline.split(" ")[-1])
        ...
        print_with_backtrace(exc_type, exc, new_tb)
    except:
        # Note: this code should never be reached
        print_with_backtrace(exc_type, exc, extracted_tb)
        raise
...
if __name__ == '__main__':
    try:
        ...
        exc_type, exc, tb = sys.exc_info()
        print_with_backtrace(exc_type, exc, traceback.extract_tb(tb))
        sys.exit(1)
