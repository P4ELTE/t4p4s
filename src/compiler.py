...
def translate_line_with_insert(file, line_idx, line, indent_str):
    ...
    if is_nonempty_line and line_idx is not None:
        if args['desugar_info'] == "comment":
            ...
            # content += " // {}@{}".format(os.path.basename(file), line_idx)
...
def add_gen_in_def(code_lines, orig_file):
    ...
    if is_inside_gen:
        ...
        #     orig_file = "(unknown file)"
        # addError("compiling " + orig_file, "unended generator method at the end of file {}".format(file))

    return new_lines


def translate_file_contents(file, orig_file, code, indent_str="    ", prefix_lines=""):
    ...
    new_lines += """
# Autogenerated file (from {}), do not modify directly.
...
def sugar(file, line):
    global file_sugar_style
    if file_sugar_style[-1] == 'line_comment': return ' // ' + file + '@' + str(line) + '\\n'
    if file_sugar_style[-1] == 'inline_comment': return '/* ' + file + '@' + str(line) + '*/'
    return ''
...
""".format(file, indent_str).splitlines()
    ...
    code_lines = add_gen_in_def(code_lines, orig_file)
    ...
    for idx, code_line in code_lines:
        ...
        if has_translatable_comment.match(code_line):
            ...
            if args['desugar_info'] == "comment":
                new_line += " # {}@{}".format(os.path.basename(file), idx)
...
def generate_code(file, genfile, localvars={}):
    ...
    with open(file, "r") as orig_file:
        code = orig_file.read()
        code = translate_file_contents(file, orig_file, code)
...
def generate_desugared_src(hlir, hlir16):
    ...
    for fromfile in glob.glob("src/utils/*.sugar.py"):
        ...
        with open(fromfile, "r") as orig_file:
            ...
            code = translate_file_contents(fromfile, fromfile, code, prefix_lines=prefix_lines)
