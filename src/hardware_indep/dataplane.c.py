    ...
    for f in sortedfields:
        if 'header' in f:
            ...
            if f.size <= 32 or f.size % 8 == 0:
                byte_width = (f.size+7)//8
                #[ memcpy(key, ((control_locals_{table.control.name}_t*) pd->control_locals).${f}, ${byte_width});
                #[ key += ${byte_width};
            else:
...
for table in hlir.tables:
    ...
    if 'key' in table:
        ...
        if table.key_length_bits == 0:
            ...
            #[     debug(" " T4LIT(XXXX,status) " Lookup on no-key table $$[table]{table.name}/" T4LIT(${table.matchType}) ": $$[action]{}{%s} (default)\n",
            #[               action_names[entry->action.action_id]
            #[               );
        else:
            #[     dbg_bytes(key, table_config[TABLE_${table.name}].entry.key_size,
            #[               " " T4LIT(????,table) " Table lookup $$[table]{table.name}/" T4LIT(${table.matchType}) "/" T4LIT(%dB) ": %s",
            #[               ${table.key_length_bytes},
            #[               ${table.key_length_bytes} == 0 ? "$$[bytes]{}{(empty key)}" : "");

            #[     table_entry_${table.name}_t* entry = (table_entry_${table.name}_t*)${table.matchType}_lookup(tables[TABLE_${table.name}], (uint8_t*)key);
        ...
        if action:
            ...
            #[    debug(" :::: Lookup on keyless table " T4LIT(${table.name},table) ", default action is " T4LIT(%s,action) "\n", action_names[entry->action.action_id]);
            #[    bool hit = true;
...
for h in hlir.header_instances:
    if not h.urtype.is_metadata:
...
for h in hlir.header_instances:
    if not h.urtype.is_metadata:
        ...
        #{ {
        #[     .type = ${h.id},
        #[     .length = hdr_infos[${h.id}].byte_width,
        #[     .pointer = NULL,
...
#[         .pointer = malloc(hdr_infos[HDR(all_metadatas)].byte_width * sizeof(uint8_t)),
#[         .var_width_field_bitwidth = 0
#}     };
#} }
...
#[     uint32_t res32;
#[     MODIFY_INT32_INT32_BITS_PACKET(pd, HDR(all_metadatas), FLD(all_metadatas,drop), false);
#} }
