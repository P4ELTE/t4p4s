...
void set_default_action_ipv4_lpm()
{
	...
	strcpy(ap->name, "portmask");
	memcpy(ap->bitmap, &portmask, 4);
	ap->length = 4*8+0;
...
void set_default_action_nexthops(uint8_t dmac[6], uint8_t smac[6], uint8_t port)
{
	char buffer[2048];
	struct p4_header* h;
	struct p4_set_default_action* sda;
