...
if [ ${success_count} -eq $((${#testcases[@]}-${#skipped[@]})) ]; then
    ...
    fail_codes[12]="Data from the control plane was wrong"
    fail_codes[13]="Could not init crypto device"
    fail_codes[14]="Tried to send digest without an active control plane"
